...
export class RouteBoxComponent implements OnInit {
  ...
  loadSources() {
    ...
    this.sources$ = concat(
      ...
      this.sourcesInput$.pipe(
        ...
        switchMap(term => {
          const res = this.locationService.FindByLocationName(term).pipe(
            tap(() => this.sourcesLoading = false)
          )
          return res;
        })
      )
  ...
  handleConfirm() {
    ...
    this.router.navigate(['/pages/result/' + this.selectedSource?.id + '/'+ this.selectedDestination?.id])
  }
}
